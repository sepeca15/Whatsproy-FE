...
const orderSlice = createSlice({
    ...
    reducers:{
        ...
        onConfirmOrder: (state, {payload}) => {
            ...
            const newStateOrdersPending = state.ordersPending.filter((order: any) => order.orderId !== newOrderConfirmed.orderId);
            const newStateOrdersFinished = [...state.ordersFinished, newOrderConfirmed];
        ...
        onDeleteOrder: (state, {payload})=> {
            ...
            if(key === 'pending') {                
                const newState = state.ordersPending.filter((order: any) => order.orderId !== orderId);
                state.ordersPending= newState
                
            } else {                
                const newState = state.ordersFinished.filter((order: any) => order.orderId !== orderId);
...
export const {onLoadOrdersPending, onLoadOrdersFinished, onConfirmOrder, onDeleteOrder, onLoadingApi, onFinishLoadingApi, onAddOrderPending } = orderSlice.actions
