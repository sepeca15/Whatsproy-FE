...
const MenusUpload: React.FC = () => {
    ...
    const [detectedProducts, setDetectedProducts] = useState<DetectedProductData[]>([])
    ...
    const processImageToText = async (imageUrl: string) => {
        ...
        try {
            ...
            if (resp.ok && resp.data) {
                ...
                const mappedProducts: DetectedProductData[] = resp.data.map((product: any, index: number) => ({
                    ...
                    plazoDuracionEstimadoMinutos: product.plazoDuracionEstimadoMinutos || 20,
    ...
    const updateProduct = (index: number, field: keyof DetectedProductData, value: any) => {
        setDetectedProducts((prev) => prev.map((product, i) => (i === index ? { ...product, [field]: value } : product)))
    }

    const handleConfirmProducts = async () => {
        const confirmedProducts = detectedProducts.filter((_, index) => selectedProducts.has(index))
        ...
        try {
            ...
            const response = await api.menu.create(uploadedImageUrl, `menu-${menus.length + 1}`)
    ...
    const renderEmptyState = () => (
        <Animatable.View animation="fadeIn" style={styles.emptyStateContainer}>
            <Text style={styles.emptyStateTitle}>
                <FormattedMessage id="noMenusYet" defaultMessage="No tienes menús aún" />
            </Text>
            ...
            {(uploadingImage || processingImage) && (
                <View style={styles.uploadingContainer}>
                    ...
                    <Text style={styles.uploadingText}>
                        {uploadingImage ? (
                            <FormattedMessage id="uploadingImage" defaultMessage="Subiendo imagen..." />
                        ) : (
                            <FormattedMessage id="processingMenuImage" defaultMessage="Procesando imagen del menú..." />
                        )}
                    </Text>
                </View>
            )}
        ...
        <Animatable.View key={menu.id} animation="fadeInUp" style={styles.menuCard}>
            ...
            <View style={styles.menuContent}>
                ...
                <View style={styles.menuActions}>
                    <TouchableOpacity style={styles.deleteButton} onPress={() => deleteMenu(menu.id)}>
    ...
    const renderProductConfirmation = () => (
        <ScrollView style={styles.productsList}>
            <View style={styles.confirmationHeader}>
                <TouchableOpacity onPress={handleCancelConfirmation} style={styles.backButton}>
                    <Icon name="arrow-left" size={20} color={Colors.light.text} />
                </TouchableOpacity>
                <Text style={styles.confirmationTitle}>
                    <FormattedMessage id="confirmDetectedProducts" defaultMessage="Confirmar Productos" />
                </Text>
            ...
            <View style={styles.bulkActionsContainer}>
                <Text style={styles.sectionTitle}>
                    <FormattedMessage id="detectedProducts" defaultMessage="Productos Detectados" />
                </Text>
                <View style={styles.bulkActions}>
                    <TouchableOpacity style={styles.bulkActionButton} onPress={selectAllProducts}>
                        <Text style={styles.bulkActionText}>
                            <FormattedMessage id="selectAll" defaultMessage="Seleccionar todos" />
                        </Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.bulkActionButton} onPress={selectNoProducts}>
                        <Text style={styles.bulkActionText}>
                            <FormattedMessage id="selectNone" defaultMessage="Deseleccionar todos" />
                        </Text>
                    </TouchableOpacity>
                </View>
            </View>
                ...
                <View style={styles.summaryContainer}>
                    <Text style={styles.summaryText}>
                        <FormattedMessage
                            ...
                            values={{ confirmed: selectedProducts.size, total: detectedProducts.length }}
                        />
                    </Text>
                </View>

                <View style={styles.actionButtons}>
                    <TouchableOpacity style={styles.cancelButton} onPress={handleCancelConfirmation} disabled={uploadingImage}>
                    ...
                    <TouchableOpacity
                        style={[
                            ...
                            (selectedProducts.size === 0 || uploadingImage) && styles.confirmButtonDisabled,
                        ]}
                    ...
                    >
                        <Text style={styles.confirmButtonText}>
                            {uploadingImage ? (
                                <FormattedMessage id="processing" defaultMessage="Procesando..." />
                            ) : (
        ...
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={styles.loadingText}>
                    <FormattedMessage id="loadingMenus" defaultMessage="Cargando menús..." />
                </Text>
            </View>
        )
    }
            ...
            {menus.length === 0 ? (
                ...
                <>
                    <View style={{ paddingHorizontal: 20, paddingVertical: 12 }}>
                        <InformativeText text={<FormattedMessage id="informativeMenuData" />} />
                    </View>
                    <View style={styles.header}>
                        <Text style={styles.headerTitle}>
                            <FormattedMessage id="dailyMenus" defaultMessage="Menús Diarios" />
                        </Text>
                        <TouchableOpacity
                    ...
                    <ScrollView style={styles.menusList} showsVerticalScrollIndicator={false}>
