  ...
  return (
    <Box bg="white" borderRadius="lg" p={4} shadow={2} mb={4}>
      <Text style={{ fontSize: 16, fontWeight: "bold", marginBottom: 10 }}>
        <FormattedMessage id="profile.recentActivity" defaultMessage="Actividad reciente" />
      </Text>
        ...
        {activities.map((activity) => (
          <HStack key={activity.id} space={3} alignItems="center">
            <Box bg={`${activity.iconColor}20`} p={2} borderRadius="full">
              <Icon as={Ionicons} name={activity.icon} size="sm" color={activity.iconColor} />
            </Box>
            <VStack flex={1}>
              <Text style={{ fontSize: 14, fontWeight: "bold" }}>{activity.title}</Text>
              <Text style={{ fontSize: 12, color: "#666" }}>{activity.description}</Text>
            </VStack>
