    ...
    <Box bg="white" borderRadius="lg" p={4} shadow={2} mb={4}>
      <HStack justifyContent="space-between" alignItems="center" mb={10}>
        <Text style={{ fontSize: 16, fontWeight: "bold" }}>
          <FormattedMessage id="sales.trend" defaultMessage="Tendencia de ventas" />
        </Text>
        ...
        <Select
          ...
          _selectedItem={{
            bg: `rgba(7, 94, 84, 0.1)`,
            endIcon: <Icon as={Ionicons} name="checkmark" size="xs" color="#075e54" />,
          }}
        >
      ...
      <Box height={150} mb={4}>
        <HStack justifyContent="space-between" height="100%" alignItems="flex-end">
        ...
        return (
          <VStack key={index} alignItems="center" space={1}>
            <Text style={{ fontSize: 7, marginBottom: 2 }}>${sale.toLocaleString()}</Text>
            <Box
          width={barWidth}
      ...
      <HStack justifyContent="space-between">
        <VStack>
          ...
          <Text style={{ fontSize: 14, fontWeight: "bold" }}>${Math.max(...monthlySales).toLocaleString()}</Text>
        </VStack>
        ...
        <VStack>
          ...
          <Text style={{ fontSize: 14, fontWeight: "bold" }}>
            ${toStrictInteger(monthlySales.reduce((a, b) => a + b, 0) / monthlySales.length).toLocaleString()}
          </Text>
        </VStack>
        ...
        <VStack>
          ...
          <Text style={{ fontSize: 14, fontWeight: "bold" }}>${Math.min(...monthlySales).toLocaleString()}</Text>
        </VStack>
      </HStack>
    </Box>
  )
