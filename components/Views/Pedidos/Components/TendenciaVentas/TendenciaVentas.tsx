...
const calculateCategoryPercentage = (category: string) => {
  const totalCategoryValue = data.reduce((sum, d) => sum + d.categories[category], 0);
  const totalValue = data.reduce((sum, d) => sum + Object.values(d.categories).reduce((a, b) => a + b, 0), 0);
  return ((totalCategoryValue / totalValue) * 100).toFixed(1);
};

const TendenciaVentasCircular: React.FC = () => {
  const [selectedSegment, setSelectedSegment] = useState<SalesSegment | null>(null);
        ...
        <G transform={`translate(${chartWidth / 2}, ${chartHeight / 2})`}>
          {segments.map((segment, index) => (
            <TapGestureHandler key={index} onActivated={() => handleSegmentPress(segment)}>
              <Path
                d={describeArc(0, 0, radius, segment.startAngle, segment.endAngle)}
                fill={segment.color}
                opacity={selectedCategory === null || selectedCategory === segment.category ? 1 : 0.3}
              />
            </TapGestureHandler>
          ))}
