...
const FilterSearch = () => {
    ...
    const handleSearchData = async () => {
        try {
            const resp = await api.order.filterByQuery(valueSearch, selectedInfoLine.nombre)
    ...
    return (
        <View style={{ paddingHorizontal: 12, paddingVertical: 6, gap: 20 }} flex={1} display={'flex'} flexDir={'column'} >
            <View w={'full'} display={'flex'} flexDir={'column'} alignItems={'center'} style={{ gap: 8 }}>
                <Select
                    dropdownIcon={<AntDesign name="filter" style={{ marginRight: 16 }} size={20} color={'gray'} />}
                    px={4}
                    py={3}
                    mt={2}
                    shadow={8}
                                    ...
                                    <Text fontSize={14} color={'gray.400'}>{valueSearch !== '' ? valueSearch : <FormattedMessage id="selectTimeFilters" />}</Text>
                                    {
                                        ...
                                        <DateTimePicker
                                            ...
                                            onChange={(_, selectedDate) => {
                                                togglePicker()
                                                if (selectedDate) {
                                                    const onlyDate = moment.tz(selectedDate, 'America/Montevideo').format('YYYY-MM-DD');
