...
const ModalCreateCategory = ({ isOpen, onClose, addCategory }: IModalCreateCategory) => {
    ...
    const onSubmit = async () => {        
        try {            
            const resp = await api.category.create({...formData, image: formData.imagen})
    ...
    return (
        <GlobalModal
            content={
                <View display={'flex'} mb={10} flexDir={'column'} alignItems={'center'} w={'full'}>
                    <View display={'flex'} flexDir={'column'} alignItems={'center'}>
                        <View w={'120px'} h={'120px'} bg={'gray.100'} rounded={'full'} alignItems={'center'}>
                            {
                                selectedImage &&
                                <Image alt="selectedimage" source={{ uri: selectedImage }} w={'full'} h={'full'} rounded={'full'} />

                            }
                        </View>
                        <Button size={'sm'} mt={4} bg={'black'} onPress={handleImagePick} >Upload photo</Button>
                        {
                            errors.imagen && <Text color={'red.400'} my={1}>{errors.imagen}</Text>
                        }
                    </View>
                    <InputField
                        isRequired={false}
