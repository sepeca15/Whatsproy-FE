...
const DateOrder: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = React.useState<OrderDataItem | null>(
    null
  );
  ...
  const updateOrderData = (newOrderData: OrderDataItem) => {
    ...
    if (selectedItem?.id) {
      setOrderDate((prevState) =>
        prevState.map((item) =>
          item.id === selectedItem.id ? newOrderData : item
        )
      );
    } else {
    ...
    <View style={styles.container}>
                      ...
                      <View style={styles.fieldTypeContainer}>
                        <View
                          style={[
                            ...
                            {
                              backgroundColor:
                                getFieldTypeColor(item.tipo) + "20",
                            },
                          ]}
                        >
                          <MaterialIcons
      ...
      {stateModal && (
        <ModalCreateOrderDate
          updateOrder={updateOrderData as any}
