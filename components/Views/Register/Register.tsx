  ...
  const validateError = (currentStep: number) => {
    ...
    if (currentStep === 1) {
      if (!formData.nombre.trim())
        error.nombre = intl.formatMessage({
          id: "validName",
          defaultMessage: "Please enter a valid name",
        });
      if (!formData.descripcion.trim())
        error.descripcion = intl.formatMessage({
          id: "validDescription",
          defaultMessage: "Please enter a valid description",
        });
      if (!formData.timeZone.trim())
        error.timeZone = intl.formatMessage({
          id: "validTimeZone",
          defaultMessage: "Please select a valid time zone",
        });
    }
    ...
    if (currentStep === 2) {
      ...
      if (!formData.tipoServicioId)
        error.tipoServicioId = intl.formatMessage({
          id: "validServiceType",
          defaultMessage: "Please select a valid service type",
        });
    }

    if (currentStep === 3) {
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.userEmail))
        error.userEmail = intl.formatMessage({
          id: "validEmail",
