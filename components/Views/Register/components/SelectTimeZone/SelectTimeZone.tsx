        ...
        <View>
            <TouchableOpacity onPress={handleModal} style={styles.container}>
                <Text style={styles.buttonText}>
                    {timeZoneSelected ? timeZoneSelected : intl.formatMessage({ id: "selectTimeZone", defaultMessage: "Select a time zone" })}
                </Text>
            </TouchableOpacity>
            <Modal style={styles.modalContainer} isOpen={stateModal} onClose={handleModal}>
                <Center>
                    <Modal.Content style={styles.modalContent}>
                        <InputField
                            label={intl.formatMessage({ id: "searchTimeZone", defaultMessage: "Search Time Zone" })}
                            placeholder={intl.formatMessage({ id: "enterTimeZone", defaultMessage: "Enter time zone..." })}
                            onChangeText={(text) => setValueSearch(text)}
                        />
                        <ScrollView style={styles.containerScroll}>
                            {
                                valueSearch && valuesFiltered.length === 0 ? (
                                    <Text>{intl.formatMessage({ id: "noResultsFound", defaultMessage: "No results found" })}</Text>
                                ) : (
