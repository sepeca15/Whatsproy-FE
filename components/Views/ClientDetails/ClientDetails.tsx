...
const ClientDetails = ({ }) => {
    const { clientDataString }: { clientDataString: string } = useLocalSearchParams();
    ...
    console.log(clientData.pedido[0]);
    ...
    return (
        <Box flex={1}>
            ...
            <ScrollView p={4}>
                <View bg={'white'} shadow={'3'} p={4} rounded={'md'}>
                    <View display={'flex'} flexDir={'row'} alignItems={'center'} >
                        <View display={'flex'} flex={1} flexDir={'row'} alignItems={'center'} style={{ gap: 12 }}>
                            ...
                            <View display={'flex'} flexDir={'column'}>
                                <Text color="teal.600" bold>Nombre: <Text color="teal.800">{clientData?.nombre}</Text></Text>
                                <Text color="teal.600" bold>Teléfono: <Text color="teal.800">{clientData?.telefono}</Text></Text>
                            </View>
                        </View>
                    </View>
                    ...
                    <VStack mt={4}>
                        ...
                        <Text>Promedio: ${(clientData?.totalGenerated / clientData?.pedido?.length).toFixed(2)}</Text>
                    </VStack>
                </View>
                {clientData?.pedido?.map((pedido: any) => {
                    return <View my={1} key={pedido?.id}>
