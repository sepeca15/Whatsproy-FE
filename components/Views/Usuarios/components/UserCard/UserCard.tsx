    ...
    const onDeleteUser = async () => {
        try {
            ...
            if (resp.ok) {
                ...
                showToast({
                    title: intl.formatMessage({ id: "userDeleted", defaultMessage: "User deleted successfully" }),
                    status: 'success'
                });
            }
        } catch (error: any) {
            ...
            <View style={styles.data}>
                <View style={styles.row}>
                    ...
                    <View style={styles.Column}>
                        <Text style={styles.textName}>{user.id === infoUser.id ? intl.formatMessage({ id: "me", defaultMessage: "Me" }) : infoUser.nombre}</Text>
                    ...
                    <View style={styles.statusUser}>
                        ...
                        <Text style={{ color: '#8e8e95' }}>{infoUser.activo ? intl.formatMessage({ id: "active", defaultMessage: "Active" }) : intl.formatMessage({ id: "inactive", defaultMessage: "Inactive" })}</Text>
                    </View>
                </View>
            </View>
            {
                allowManage ?
                    ...
                    :
                    <Text style={{ alignSelf: 'flex-end' }}>{intl.formatMessage({ id: "noPermission", defaultMessage: "You do not have permission to manage users" })}</Text>
            }
            <ModalConfirmAction isOpen={stateModal} onContinue={onDeleteUser} onClose={toggleModal} message={intl.formatMessage({ id: "confirmDeleteUser", defaultMessage: "Do you want to delete the selected user?" })} title={intl.formatMessage({ id: "deleteUser", defaultMessage: "Delete User" })} />
        </View>
    );
};
